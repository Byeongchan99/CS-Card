...
문제는 델리게이트 필드를 `public`으로 열면 바깥에서 아무거나 다 된다는 것 — ① `enemy.OnHit()`으로 아무 때나 강제 발동, ② `enemy.OnHit = null`/다른 값으로 남이 `+=`로 등록해둔 구독 목록을 통째로 덮어씀. 여러 시스템(UI·사운드·도전과제)이 구독해뒀는데 누가 `=`로 덮으면 나머지가 조용히 사라지고, 발동 권한도 아무나 가짐. 캡슐화가 깨짐.
...
먼저 층위 — "함수를 값으로 다루는" 능력은 델리게이트(함수를 담는 그릇)의 것이고, 람다는 그 값을 그 자리에서 짧게 만들어내는 문법. 람다가 새 능력을 준 게 아니라 델리게이트 값 만들기를 간결하게 함(이름 붙인 메서드 → 익명 메서드 → 람다로 발전). 람다 고유의 값은 셋 — 간결함(타입·`return`·중괄호 생략), 인라인(쓰는 자리에 바로), 캡처(주변 지역 변수를 매개변수 없이 끌어씀).
...
이 셋이 콜백·LINQ·이벤트 구독을 실용화함 — 로직을 쓰는 자리에 짧게 붙이고 문맥을 데려가니 함수형·선언적 스타일(`Where`·`Select` 체인)이 읽을 만해짐. 대가는 매개변수 타입이 코드에 안 박혀 문맥·이름에 의존한다는 것, 그리고 델리게이트 객체(+캡처 시 숨은 상자)가 힙에 할당된다는 것.

로컬 함수는 메서드 안에 이름 붙여 선언한 함수 — 델리게이트로 넘기지만 않으면 컴파일러가 평범한 숨은 메서드로 컴파일해 델리게이트 객체·할당이 없음. 그래서 매 프레임 도는 핫 패스의 내부 헬퍼엔 로컬 함수가 유리(캡처해도 `ref`로 가볍게 넘김). 재귀·`yield return`·`out` 매개변수도 됨. 단 로컬 함수는 값이 아니라, 콜백·LINQ·이벤트처럼 함수를 값으로 넘기는 자리엔 못 들어감(넘기는 순간 어차피 델리게이트로 변환).
