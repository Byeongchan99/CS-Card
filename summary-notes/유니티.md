...
`Instantiate(prefab, pos, rot)`는 처음부터 그 위치로 생성. `Instantiate` 후 `transform.position` 대입은 "기본 위치 생성 → 다시 이동"이라 Transform 갱신(자식 월드 좌표 재계산, 물리 통보)이 두 번 일어날 수 있음. 대량 스폰에서 차이가 쌓임.

> [!tip] 복습 포인트
> 인자로 넘기면 갱신 1회, 생성 후 대입은 2회.
...
엔진(C++)이 각 MonoBehaviour의 `Update`를 매 프레임 하나씩 호출하는데, C++↔C# 경계 넘기에 건당 오버헤드가 있음. 매니저 하나가 리스트를 돌면 경계 호출이 1번으로 줄고 나머지는 C# 내부 루프. 빈 `Update`도 호출 비용이 있으니 안 쓰면 지울 것.

> [!tip] 복습 포인트
> 핵심은 경계 호출 횟수 감소. "매 프레임 콜백을 가진 인스턴스 수"를 줄이는 것.

## Animator StringToHash

`SetTrigger("Jump")`는 매 호출마다 문자열을 내부적으로 해시로 변환·조회함. `StringToHash`로 해시를 미리 구해 캐싱하고 정수로 넘기면 반복되는 문자열 해싱을 건너뜀.

> [!tip] 복습 포인트
> GC가 아니라 문자열 해싱 비용이 핵심.
...
정점 많은 동일 메시가 다수면 GPU Instancing.

> [!tip] 복습 포인트
> Dynamic은 정점 수 제한이 있음. 정점이 많으면 GPU Instancing.
